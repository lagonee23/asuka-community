...
function JapaneseTest({ db, currentUser, appId }) {
  ...
  const fetchWords = useCallback(async () => {
    ...
    try {
      ...
      if (fetchedWords.length === 0) {
        setError("시험을 보려면 최소 1개의 일본어 단어가 필요합니다.");
      } else {
  ...
  return (
    <section className="w-full max-w-6xl mx-auto bg-white rounded-xl shadow-lg p-6 sm:p-8 flex-1 flex flex-col items-center justify-center">
      <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-6 border-b-2 border-green-200 pb-2 text-center">
        일본어 단어 시험
      </h2>
